...
public class SnmpTargetMIB implements MOGroup, CounterListener,
  ...
  private Map<OctetString, Set<MOTableRow>> snmpTargetAddrTagIndex = new Hashtable<OctetString, Set<MOTableRow>>();
  ...
  public Collection<MOTableRow> getTargetAddrRowsForTag(OctetString tag) {
    Collection<MOTableRow> l = snmpTargetAddrTagIndex.get(tag);
    if (l == null) {
      return Collections.<MOTableRow>emptySet();
    }
    else {
      synchronized (l) {
        l = new ArrayList<MOTableRow>(l);
      }
    }
    return l;
  }
  ...
  protected void removeRowFromTargetAddrTagIndex(MOTableRow removedRow) {
    ...
    if ((tags != null) && (this.snmpTargetAddrTagIndex != null)) {
      for (OctetString item : tags) {
        Collection<MOTableRow> indexRows = this.snmpTargetAddrTagIndex.get(item);
        if (indexRows != null) {
    ...
    public void rowStatusChanged(RowStatusEvent event) {
      if (event.getNewStatus() == RowStatus.destroy) {
        ...
        synchronized (snmpTargetAddrEntryModel) {
          for (Iterator<MOTableRow> it = snmpTargetAddrEntryModel.iterator(); it.hasNext(); ) {
    ...
    private void updateIndex(Set<OctetString> remove, Set<OctetString> tags) {
      if (remove != null) {
        for (OctetString next : remove) {
          Collection<MOTableRow> list = snmpTargetAddrTagIndex.get(next);
          if (list != null) {
            synchronized (list) {
              if (!list.remove(this)) {
      ...
      for (OctetString next : tags) {
        Set<MOTableRow> list = snmpTargetAddrTagIndex.get(next);
        if (list == null) {
          list = new HashSet<MOTableRow>();
        }
        synchronized (list) {
          if (!list.add(this)) {
