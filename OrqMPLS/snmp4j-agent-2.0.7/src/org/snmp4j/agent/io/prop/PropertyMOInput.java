...
public class PropertyMOInput implements MOInput {
  ...
  public IndexedVariables readIndexedVariables() throws IOException {
    if (data == null) {
      data =  new DataInfo<IndexedVariables>(oids.curContext);
      scanDataIndexVariables(properties, oids.curOID.getOID(), data);
    }
...
//        state++;
  ...
  public Variable readVariable() throws IOException {
    if (data == null) {
      data = new DataInfo<Variable>(oids.curContext);
      scanDataVariable(properties, oids.curOID.getOID(), data);
    }
